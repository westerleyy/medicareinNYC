...
nyc_hospital_shp <- st_read(paste(path, nyc_hospital_folder,"hospitals_thiessen_polyarea.shp",sep = ""), stringsAsFactors = F)
```
...
# nyc_hospitals_address_geocode[11,1:2] <- c(-73.958931,40.7627183)
# nyc_hospitals_address_geocode[16,1:2] <- c(-73.9843091,40.7331083)
...
nyc_hospitals_spdf <- SpatialPointsDataFrame(coords = nyc_hospitals_address_geocode, data = nyc_hospitals_address, proj4string = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))
...
borough_list <- nyc_medicare_2017 %>%
  ...
  unique()
catchment_area <- (nyc_hospital_shp$poly_area)/27878350.09766
...
```
Here we can see that the hospitals with the largest catchment areas are in Queens, SI, and the Bronx.   

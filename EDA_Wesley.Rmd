...
nyc_hospital_shp <- st_read(paste(path, nyc_hospital_folder,"hospitals_thiessen.shp",sep = ""), stringsAsFactors = F)
nyc_hospital_points <- st_read("./nyc_hospitals/nyc_hospitals.shp", stringsAsFactors = F)
```

#### Hospitals in New York: Where
```{r New York}
# function to parse through each medicare df for just those in NYC
nyc_medicare <- function(medicare_df, zip_codes = nyc_zip_codes, zipBx = 104, zipBk = 112, zipMn = 100, zipSI = 103, health_plus = nyc_health_plus){
  ...
  mutate(zip_3 = substr(`Provider Zip Code`, 1, 3),
         ...
         health_plus = ifelse(`Provider Name` %in% health_plus, "Y", "N"))
  nyc_medicare
}
...
```{r time-series, fig.height=10, fig.width=10}
top_n_drg <- function(medicare_df, n, yr, procedure_count = procedures_count){
  ...
  target_df <- medicare_df %>%
    ...
    select(`Provider Name`, DRG_num, `Total Discharges`:`Average Medicare Payments`, borough, health_plus) %>%
    mutate(Average_Non_Medicare_Payments = `Average Total Payments` - `Average Medicare Payments`,
           Year = yr,
...
ggplot(nyc_medicare_2014_2017_topfive_df_timeseries, aes(x = Discharge_Year, y = YoY_Change)) + 
  geom_point() + 
  geom_path(alpha = 0.5) + 
